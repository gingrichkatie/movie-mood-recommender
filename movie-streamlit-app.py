...
# -----------------------------
# Styles (cleaner contrasts + spotlights)
# -----------------------------
st.markdown("""
<style>
/* Background: dark cinema wall */
.stApp {
  background: linear-gradient(180deg, #0b0f14 0%, #0b0f14 100%);
}

/* Keep default Streamlit widget colors; only adjust headings */
...
.app-hero{
  ...
  border:1px solid rgba(255,255,255,.55); box-shadow:0 12px 28px rgba(0,0,0,.25);
...
.app-hero h1{
  display:inline-block; padding:10px 20px; border-radius:12px; font-weight:800; color:#0f141a;
...
/* Section panel (light “box”) with soft wall spotlights behind */
.section-panel{
  position:relative; border-radius:16px; background:#ffffff; color:#0f141a;
  border:1px solid rgba(0,0,0,.06); padding:18px 18px 22px; margin:8px 0 20px;
  box-shadow:0 10px 30px rgba(0,0,0,.18);
}
.section-panel::before{
  content:""; position:absolute; inset:-22px; z-index:-1; border-radius:24px; filter:blur(2px);
  background:
    radial-gradient(900px 260px at 18% -22%, rgba(255,233,165,.7) 0%, rgba(255,233,165,0) 65%),
    radial-gradient(900px 260px at 82% -22%, rgba(163,216,255,.65) 0%, rgba(163,216,255,0) 65%);
}

/* Movie cards */
.movie-card{ border-radius:16px; padding:14px; border:1px solid rgba(0,0,0,.08); background:#fff; }
.movie-title{ font-weight:700; font-size:1.05rem; margin:2px 0 4px; color:#0f141a; }
.movie-meta{ font-size:.92rem; color:#1c232c; }
.reason{ font-size:.95rem; margin-top:8px; color:#1e232a; }
.chip{ display:inline-block; font-size:.80rem; padding:2px 8px; margin-right:6px;
       border-radius:999px; border:1px solid rgba(0,0,0,.08); background:#f4f7fb; color:#0f141a; }
...
"""
  resp = client.chat.completions.create(
    model="gpt-4o-mini",
    messages=[{"role":"system","content":"You are a concise movie recommendation assistant."},
  ...
  try:
    ...
    cleaned = [{"title": (d.get("title") or "").strip(),
                "reason": (d.get("reason") or "").strip()} for d in data if d.get("title")]
    return cleaned[:5]
  except Exception:
...
with tab2:
  ...
  if st.button("📥 Load Data"):
    try:
      ...
      if not movies: st.warning("No data found for that genre.")
      else:
        ...
        # ✅ Render poster URLs as images
        col_cfg = {}
        if show_posters:
          col_cfg["poster"] = st.column_config.ImageColumn("poster", help="TMDB poster", width="small")
